...
public class DayPanel extends JComponent {
    ...
    private HashMap<Date, Boolean> hasPhotoMap = new HashMap<>();
...
//        cselected = UIManager.getLookAndFeel().getDefaults().getColor("textHighlight");
    ...
    public void paint(Graphics g) {
        Component comp;
        if (hasPhotoMap.get(date) != null) {
            comp = renderer.getDayRenderer(this, date, data, isSelected(), working,
                    (isEnabled() && (isComponentEnabled())));
            if (enabled)
                ((JLabel) comp).setBorder(BorderFactory.createMatteBorder(5, 5, 5, 5, Color
                        .decode("#001E28")));
        } else {
            comp = renderer.getDayRenderer(this, date, data, isSelected(), working,
                    (isEnabled() && (isComponentEnabled())));
        ...
        if (isEnabled()) {
            if (CalendarUtils.isToday(date)) {
                g.setColor(Color.decode("#001E28"));
    ...
    public void setHasPhotos(boolean hasPhotos) {
        hasPhotoMap.put(date, hasPhotos);
