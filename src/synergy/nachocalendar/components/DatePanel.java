...
package synergy.nachocalendar.components;
...
import synergy.nachocalendar.event.MonthChangeEvent;
import synergy.nachocalendar.event.MonthChangeListener;
import synergy.nachocalendar.event.YearChangeEvent;
import synergy.nachocalendar.event.YearChangeListener;
import synergy.nachocalendar.model.DataModel;
import synergy.nachocalendar.model.DateSelectionModel;
import synergy.nachocalendar.model.DefaultDateSelectionModel;
...
public class DatePanel extends JPanel {
     ...
     *
     * @param showWeekNumbers true for showing week numbers
     */
...
/*            public void mouseClicked(MouseEvent e) {
    ...
    private void fireActionListenerActionPerformed(
        ...
        for (int i = 0; i < list.size(); i++) {
            ((ActionListener) list.get(i))
                    .actionPerformed(event);
        }
    }

    public MonthScroller getMonthScroller() {
        return monthscroller;
    }

    public YearScroller getYearScroller() {
        return yearscroller;
    }
    ...
    public synchronized void addMonthChangeListener(
        ...
        listenerList.add(MonthChangeListener.class,
                listener);
    }
    ...
    public synchronized void removeMonthChangeListener(
            MonthChangeListener listener) {
        listenerList.remove(
                MonthChangeListener.class, listener);
    }
    ...
    private void fireMonthChangeListenerMonthIncreased(
        ...
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == MonthChangeListener.class) {
                ((MonthChangeListener) listeners[i + 1])
