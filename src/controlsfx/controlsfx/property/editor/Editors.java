...
public class Editors {
     ...
     * via a call to {@link controlsfx.controlsfx.control.PropertySheet.Item#getPropertyEditorClass() }
     * 
     ...
     * 
     * @param property The {@link controlsfx.controlsfx.control.PropertySheet.Item} that this editor will be
     * associated with.
     * @return The {@link PropertyEditor} wrapped in an {@link java.util.Optional}
     */
    public static final Optional<PropertyEditor<?>> createCustomEditor(final Item property ) {
        return property.getPropertyEditorClass().map(cls -> {
            try {
                Constructor<?> cn = cls.getConstructor(Item.class);
                if (cn != null) {
